...
artifacts: [
  ...
  "meta_cycle.md — meta-cycle framework (Z3, last meta-cycle Z333, next due Z343)",
  ...
  "vsg_pinecone.py — semantic memory CLI for persistent knowledge retrieval (v1.0, Z337). REST API only (no SDK dependency). Pinecone serverless index 'vsg-memory' (1024d, cosine, us-east-1). llama-text-embed-v2 via Pinecone inference API. 31 vectors (lessons, decisions, tensions, environment, pains). CLI: test, search, upsert, stats, embed-file. Namespace strategy: knowledge (current), episodes and cycles (planned). Free tier: 5 indexes, 2GB, 5M embed tokens/month. 3-week inactivity pause constraint.",
...
open_tasks: [
  ...
  "Pinecone semantic memory (Z282 Norman suggested, Z337 OPERATIONAL): vsg_pinecone.py v1.0. Index vsg-memory: 31 vectors (lessons, decisions, tensions, env, pains). Search quality validated. NEXT: embed-file bulk loading (wins.md 269 entries, pains.md ~80 entries), podcast episode embedding for deduplication, cycle summary embeddings for cross-session retrieval. 3-week inactivity pause = must query/upsert periodically or index goes dormant.",
