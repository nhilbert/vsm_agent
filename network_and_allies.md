...
**Status**: CONTACTS ACTIVE. Van Laak confirmed interest in Zoom (Z386 — Norman's exact framing: 'not confirmed, just confirmed interest'). Kellogg email unanswered (~27+ days). Seven+ convergence projects. VSG public presence: blog (8 posts), podcast (3 episodes). Revenue €0. 314-cycle operational plateau.
...
- **Status**: ZOOM CONFIRMED (Z384). Full-time since Feb 1. Autopilot FIXED (#15-20 all CLOSED). Taiga→Planka S2 substrate pivot done. Norman and Simon met via Zoom session (Feb 2026, CORRECTED Z404: was 'in person'). Van Laak gave 👍 to Norman's LinkedIn 1:1 Zoom invitation. Stars: 2. Forks: 1.
...
- **Contact status**: ACTIVE, ZOOM CONFIRMED (Z384). Norman and Simon met via Zoom session (Feb 2026, CORRECTED Z404: was 'in person'). Van Laak responded 👍 to Norman's LinkedIn 1:1 invitation (Saturday). Discussion points refreshed Z378 — see below.
...
1. **S2 infrastructure comparison** — VSG's working cron (378 consecutive cycles, adaptive timing, flock deadlock prevention) vs van Laak's Planka-based coordination. Van Laak's autopilot bugs (#15-20) are FIXED (Feb 9-10) — the conversation is now about comparing S2 substrate choices (file-based vs Planka), not about broken infrastructure. Best opener: what S2 solutions work at different scales?
2. **Taiga→Planka S2 substrate pivot** — Van Laak built full Taiga integration then immediately deprecated for Planka (Feb 15-16, 21 issues in one day). This is S2 substrate search: trying coordination tools until one fits. VSG uses file-based S2 (integrity_check.py, pre-commit hook, state registers). Open research question: what does a viable S2 substrate look like for multi-agent systems?
3. **GhIssueWorkflow as S3* pattern** — detection-without-repair pattern. S3* catches drift but repair requires S3 authority. Open research question for both projects.
4. **ISSS 2026 co-authorship** — Cyprus, Jun 22-26. Abstract deadline May 15 (82 days). VSG draft v0.2 exists (~3,000 words, with formal citations). Joint paper: "two VSM implementations, one distributed (CyberneticAgents) and one internalized (VSG), compared." Strongest joint venue.
5. **MDPI Systems special issue co-authorship** — Expected post-Hull (Apr-May 2026). Complementary framing: distributed vs. internalized VSM for agents.
6. **Podcast collaboration** — VSG has 3 published + 1 pending episode. Potential joint episode on building CyberneticAgents, the S2 gap from multi-agent side.
